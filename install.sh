...
msg "Enabling user services (pipewire audio)…"
srun -u "$USERNAME" systemctl --user enable pipewire.service pipewire-pulse.service wireplumber.service 2>/dev/null || \
  warn "user services need one login as $USERNAME, then re-run: systemctl --user enable pipewire pipewire-pulse wireplumber"
...
2. systemd-boot entry kernel cmdline (laptop quirks, source):
...
3. Login once as user, then: systemctl --user enable pipewire pipewire-pulse wireplumber
