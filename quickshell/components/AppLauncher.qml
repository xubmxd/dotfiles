...
Item {
    ...
    Process {
        ...
        stdout: StdioCollector {
            onStreamFinished: {
                ...
                root.rebuildResults()
                favoriteScanProc.running = false
                favoriteScanProc.running = true
            }
        }
        onExited: (exitCode) => {
            if (exitCode !== 0) {
    ...
    // Called by the shell Loader after this component has actually been
    // instantiated. Deferring focus by one event turn avoids racing the
    // layer-shell keyboard-focus transition on first open.
    ...
    function retainFavoritesOnly() {
        if (!root.fullApplicationsLoaded)
            return
    ...
    function ensureFullApplicationsLoaded() {
        if (root.fullApplicationsLoaded || root.fullScanRequested)
            return

        root.fullScanRequested = true
        scanProc.running = false
        scanProc.running = true
    }

    function refreshApplications() {
        root.applicationsLoaded = false
        root.fullApplicationsLoaded = false
        root.fullScanRequested = false
        root.allApps = []
        root.refresh()
    ...
    property bool fullApplicationsLoaded: false
    property bool fullScanRequested: false
    ...
    Process {
        ...
        command: {
            ...
            const script =
                "fav=\"$(cat \"$HOME/.cache/quickshell_favorites.txt\" 2>/dev/null | sed '/^[[:space:]]*$/d')\"; " +
                "for f in " + globs + "; do " +
                "  [ -f \"$f\" ] || continue; " +
                "  name=$(grep -m1 '^Name=' \"$f\" | cut -d= -f2-); " +
                "  echo \"$fav\" | grep -Fxq \"$name\" || continue; " +
    ...
    Process {
        ...
        stdout: StdioCollector {
            onStreamFinished: {
                ...
                root.allApps = apps
                root.applicationsLoaded = true
                root.fullApplicationsLoaded = true
                root.fullScanRequested = false
                root.rebuildResults()
            }
        }
